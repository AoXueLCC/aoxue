...
export const useQuoteStore = defineStore('quote', () => {
  ...
  /* ========== 价格计算（实时联动） ========== */
  const vehiclePrice = computed(() => (truck.value ? truck.value.price : 0))
  ...
  const configStep = computed(() => {
    ...
    if (!refrigerator.value) return 1
    if (!body.value) return 2
    return accessories.value.length ? 3 : 3
  })
